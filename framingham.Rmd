...
#table(fhs_binned[,c('age', 'education', 'bmi', 'gender','diabetes','cigsPerDay')])
# check propensity scores

# 2. propensity score
prob.AW.reg<- multinom(cigsPerDay ~ age + education + bmi + gender + diabetes, data=fhs_binned)
prob.1W <- predict(prob.AW.reg, type= "probs")
# create an empty vector
prob.AW <- rep(NA, 4211)
# for bin1
prob.AW[fhs_binned$cigsPerDay=="[0, 1)"] <- prob.1W[fhs_binned$cigsPerDay=="[0, 1)","[0, 1)"]
# for bin2
prob.AW[fhs_binned$cigsPerDay=="[1, 10)"] <- prob.1W[fhs_binned$cigsPerDay=="[1, 10)","[1, 10)"]
# for bin3
prob.AW[fhs_binned$cigsPerDay=="[10, 20)"] <- prob.1W[fhs_binned$cigsPerDay=="[10, 20)","[10, 20)"]
# for bin34
prob.AW[fhs_binned$cigsPerDay=="[20, 70)"] <- prob.1W[fhs_binned$cigsPerDay=="[20, 70)","[20, 70)"]

# look at the distribution of predicted probabilities/pscore
summary(prob.AW)[1:6]

# use mean
#meanlevel<- fhs_binned %>% summarise(group_by(age, education, bmi, gender,diabetes),
#          mean=mean(cigsPerDay))
```
...
glm_fit <- glm( CHD ~ cigsPerDay + education + age + diabetes + bmi , data = fhs_binned_sample, family = "binomial")
...
### BIN 1
glm_fit_iptw_bin_1 <- glm( cigsPerDay_bin_1 ~   education + age + diabetes + bmi , data = fhs_binned_sample, family = "binomial")
...
### BIN 2
glm_fit_iptw_bin_2 <- glm( cigsPerDay_bin_2 ~   education + age + diabetes + bmi , data = fhs_binned_sample, family = "binomial")
...
### BIN 3
glm_fit_iptw_bin_3 <- glm( cigsPerDay_bin_3 ~   education + age + diabetes + bmi , data = fhs_binned_sample, family = "binomial")
...
### BIN 4
glm_fit_iptw_bin_4 <- glm( cigsPerDay_bin_4 ~   education + age + diabetes + bmi , data = fhs_binned_sample, family = "binomial")
